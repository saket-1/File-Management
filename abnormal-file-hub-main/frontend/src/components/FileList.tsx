...
export const FileList: React.FC<{ key?: number }> = () => {
  ...
  const deleteMutation = useMutation<void, Error, string>({
    ...
    onSuccess: (_, deletedFileId) => {
      const deletedFileName = files?.find(f => f.id === deletedFileId)?.name || 'File';
  ...
  if (error && !files) {
    ...
    return <div className="text-center p-4 text-red-600">Error loading files. Please try again later.</div>;
  }
